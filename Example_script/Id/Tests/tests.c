...
TEST(ECB, ProcPaddingNulls)
{
    ...
    EXPECT_EQ(procPaddingNulls((uint8_t*)&block, 3, PROC_ADD_NULLS_2), 0x0000011234567890);
...
TEST(ECB, CipherECB_proc_added_nulls_1_full_block)
{
    ...
    //encryption
    uint32_t key[8]={0xFCFDFEFF, 0xF8F9FAFB, 0xF4F5F6F7, 0xF0F1F2F3, 0x33221100, 0x77665544, 0xBBAA9988, 0xFFEEDDCC};
    EXPECT_EQ(EncryptECB(nameInputFile, nameOutputFile, key, PROC_ADD_NULLS_1), 1);
    //check cipher text
    ...
    fclose(cipherText);
    //decryption
    EXPECT_EQ(DecryptECB(nameOutputFile, nameInputFile, key, PROC_ADD_NULLS_1, 8), 1);
    //check open text
...
TEST(ECB, CipherECB_proc_added_nulls_1_not_full_block)
{
    ...
    fclose(cipherText);
    //decryption
    EXPECT_EQ(DecryptECB(nameOutputFile, nameInputFile, key, PROC_ADD_NULLS_1, 4), 1);
    //check open text
    openText = fopen("OpenText.txt", "r");
    for(int iteration = 1; iteration <= 4; iteration++)
    {
...
TEST(ECB, CipherECB_proc_added_nulls_2_not_full_block)
{
    ...
    //encryption
    uint32_t key[8]={0xFCFDFEFF, 0xF8F9FAFB, 0xF4F5F6F7, 0xF0F1F2F3, 0x33221100, 0x77665544, 0xBBAA9988, 0xFFEEDDCC};
    EXPECT_EQ(EncryptECB(nameInputFile, nameOutputFile, key, PROC_ADD_NULLS_2), 1);
    //check cipher text
    FILE* cipherText = fopen("CipherText.txt", "r");
    for(int iteration = 1; iteration <= 4; iteration++)
    {
    ...
    fclose(cipherText);
    //decryption
    EXPECT_EQ(DecryptECB(nameOutputFile, nameInputFile, key, PROC_ADD_NULLS_2, 0), 1);
    //check open text
    openText = fopen("OpenText.txt", "r");
    for(int iteration = 1; iteration <= 4; iteration++)
    {
